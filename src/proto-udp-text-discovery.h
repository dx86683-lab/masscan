...
int andromouse_probe_prepare(uint64_t cookie, const struct UdpProbeTarget *target, struct UdpPreparedProbe *result);
int andromouse_probe_classify(const unsigned char *data, unsigned length, uint64_t cookie);
